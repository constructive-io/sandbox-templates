...
function Textarea({ className, size = 'default', unstyled = false, ...props }: TextareaProps) {
	return (
		<span
			className={
				cn(
					!unstyled && [
						// Base layout
						'relative inline-flex w-full rounded-md border border-input bg-background bg-clip-padding text-base transition-shadow',
						...
						'has-focus-visible:border-ring/60 has-focus-visible:ring-[3px] has-focus-visible:ring-ring/35',
						// Invalid state
		...
		>
			<textarea
				className={cn(
					'field-sizing-content min-h-17.5 w-full rounded-[inherit] bg-transparent px-[calc(--spacing(3)-1px)] py-[calc(--spacing(1.5)-1px)] outline-none placeholder:text-subtle-foreground max-sm:min-h-20.5',
