...
CREATE FUNCTION list_memberships(
  ...
  rolname text
) AS $EOFCODE$ WITH RECURSIVE cte AS (
    SELECT
        oid
    FROM
        pg_roles
...
CREATE FUNCTION verify_constraint(
  _table text,
  _name text
...
CREATE FUNCTION verify_extension(
  _extname text
) RETURNS boolean AS $EOFCODE$
BEGIN
    IF EXISTS (
...
CREATE FUNCTION verify_membership(
  _user text,
  _role text
) RETURNS boolean AS $EOFCODE$
BEGIN
...
CREATE FUNCTION verify_security(
  _table text
) RETURNS boolean AS $EOFCODE$
BEGIN
    IF EXISTS (
...
CREATE FUNCTION verify_table_grant(
  _table text,
  _privilege text,
  _role text
) RETURNS boolean AS $EOFCODE$
...
CREATE FUNCTION verify_trigger(
  _trigger text
) RETURNS boolean AS $EOFCODE$
BEGIN
    IF EXISTS (
