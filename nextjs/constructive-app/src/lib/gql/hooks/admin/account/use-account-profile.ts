...
export function useAccountProfile(options: UseAccountProfileOptions): UseAccountProfileResult {
	const { userId, enabled = true, context = 'admin' } = options;
...
export function useUpdateUser(_defaultContext: SchemaContext = 'admin') {
...
export function useUpdateUserProfile(_defaultContext: SchemaContext = 'admin') {
