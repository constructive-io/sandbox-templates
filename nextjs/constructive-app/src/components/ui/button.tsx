...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
	({ className, variant, size, asChild, children, type = 'button', ...props }, ref) => {
		const buttonClasses = cn(buttonVariants({ className, size, variant }));

		// If asChild, render children with merged props (for wrapping Links, etc.)
		if (asChild && React.isValidElement(children)) {
			const childProps = children.props as Record<string, unknown>;
			return React.cloneElement(children as React.ReactElement<Record<string, unknown>>, {
				...props,
				className: cn(buttonClasses, childProps.className as string | undefined),
				'data-slot': 'button',
			});
		}
