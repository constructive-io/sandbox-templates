import { Clock, Eye, Globe, Link, Lock, Network, Shield, Unlock, User, UserCheck, Users } from 'lucide-react';
...
export const POLICY_TYPE_UI_CONFIG: Record<string, PolicyTypeUIConfig> = {
	...
	AuthzMembershipByField: {
		title: 'Scoped Membership',
		tagline: 'Access via membership in row entity',
		icon: Shield,
		category: 'has-module',
		hasTableModule: true,
		tableModuleType: 'DataMembershipByField',
		fieldOverrides: {
			entity_field: {
		...
		// DataMembershipByField supports custom field name via entity_field_name
	...
	AuthzOrgHierarchy: {
		...
		category: 'has-module',
		hasTableModule: true,
		tableModuleType: 'DataDirectOwnerInEntity',
		fieldOverrides: {
			direction: {
				label: 'Direction',
...
export const AUTHZ_TO_DATA_MODULE_MAPPING: Record<string, string> = {
	...
	AuthzMembershipByField: 'DataMembershipByField',
	AuthzPublishable: 'DataPublishable',
	AuthzOrgHierarchy: 'DataMembershipByField',
};
...
export function buildTableModuleData(
	...
	// For DataMembershipByField, pass through the entity_field if custom
	if (policyType.tableModuleType === 'DataMembershipByField') {
