...
export function useCreateTableWithPolicies() {
	...
	const mutation = useMutation({
		mutationFn: async (input: CreateTableWithPoliciesInput): Promise<CreateTableWithPolicyResult> => {
			...
			// Step 2.5: Create fields in parallel (for 'needs-fields' category policies)
			...
			if (category === 'needs-fields') {
				...
				await Promise.all(
					fieldsToCreate.map(async (fieldConfig) => {
			...
			// Step 3: Create all 4 CRUD policies in parallel
			setCurrentStep('creating-policies');
			...
			const policyResults = await Promise.all(
				CRUD_OPERATIONS.map(async (op) => {
					...
					return result.createPolicy?.policy?.id;
				}),
			);

			const policyIds = policyResults.filter((id): id is string => id != null);
