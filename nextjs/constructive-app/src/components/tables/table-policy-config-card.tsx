...
import { getDefaultFormValues, useCreateTableWithPolicies, usePolicyType } from './policy-hooks';
...
export const TablePolicyConfigCard: CardComponent<TablePolicyConfigCardProps> = ({
	...
	const { defaults, operations, updateDefaults, updateOperation, resetOperationToDefaults } = useCrudPolicyState();
	...
	useEffect(() => {
		if (policyType && !defaultsInitialized) {
			const defaultPolicyData = getDefaultFormValues(policyType);
			updateDefaults({ policyData: defaultPolicyData });
			setDefaultsInitialized(true);
		}
	}, [policyType, defaultsInitialized, updateDefaults]);
	...
	const handleSubmit = async () => {
		if (!policyType || !databaseId || !schemaId) return;

		// Prepare operations with injected schema fields in policyData
		...
		try {
			const result = await createTableWithPolicies({
				...
				tableModuleData,
				sharedPolicyData: injectSchemaFields(defaults.policyData, schemaId, policyType.name),
