...
export function OverflowList({
	...
	useEffect(() => {
		...
		container.addEventListener('scroll', checkForOverflow, { passive: true });
		...
		<div className={cn('relative', containerClassName)} data-testid='overflow-list'>
			{showShadows && showStartShadow && (
				<div
					className={cn(
						...
						direction === 'horizontal'
							? 'from-background top-0 left-0 h-full w-8 bg-gradient-to-r to-transparent'
							: 'from-background top-0 left-0 h-8 w-full bg-gradient-to-b to-transparent',
					)}
				/>
			)}
			...
			{navigationButtons && showStartShadow && direction === 'horizontal' && (
				<Button
					...
					size='icon'
					className={cn(
						`bg-background/80 hover:bg-background/70 active:bg-background/60 absolute z-20 h-6 w-6 rounded-full
			...
			{showShadows && showEndShadow && (
				<div
					className={cn(
						...
						direction === 'horizontal'
							? 'from-background top-0 right-0 h-full w-8 bg-gradient-to-l to-transparent'
							: 'from-background bottom-0 left-0 h-8 w-full bg-gradient-to-t to-transparent',
					)}
				/>
			)}
