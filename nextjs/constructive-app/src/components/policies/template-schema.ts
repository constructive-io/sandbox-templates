...
export type PolicyTypeDataMap = {
	...
	AuthzMembershipByField: {
		...
		permission?: string;
		[UI_POLICY_TYPE_DISCRIMINATOR_KEY]?: 'AuthzMembershipByField';
	};
...
function inferMembershipByFieldVariant(data: Record<string, unknown> | null | undefined): PolicyTypeId {
	if (data && typeof data === 'object') {
		...
		if (discriminator === 'AuthzMembershipByField') return 'AuthzMembershipByField';
	}
	if (data && typeof data === 'object' && 'membership_type' in data) return 'AuthzMembershipByField';
	return 'OwnedRecords';
}
...
export function fromBackendPolicyTypeId(
	...
	if (backendPolicyTypeId === 'AuthzMembershipByField') {
...
export function toBackendPolicyTypeId(policyTypeId: PolicyTypeId | 'AuthzComposite'): string {
	...
	if (policyTypeId === 'AuthzMembershipByField' || policyTypeId === 'OwnedRecords') {
		return 'AuthzMembershipByField';
	}
	return policyTypeId;
}
...
export const POLICY_TYPE_SCHEMAS: Record<PolicyTypeId, PolicyTypeSchema> = {
	...
	/**
	 * @deprecated Use AuthzMembershipByField with membership_type: 2 instead.
