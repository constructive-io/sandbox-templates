...
const SecurityView = dynamic(() => import('@/components/table-editor').then((m) => m.SecurityView), {
	ssr: false,
	loading: () => <SchemaBuilderEditorSkeleton />,
});
...
export function SchemasRoute() {
	...
	// Auto-select first APP table when schema loads and no table is selected
	useEffect(() => {
		if (!currentSchema || currentTable) return;

		// Find first APP category table
		const firstAppTable = currentSchema.tables.find((t) => t.category === 'APP');
		if (firstAppTable) {
