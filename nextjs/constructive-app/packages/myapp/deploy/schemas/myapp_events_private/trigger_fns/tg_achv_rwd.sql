-- Deploy: schemas/myapp_events_private/trigger_fns/tg_achv_rwd
...
CREATE FUNCTION myapp_events_private.tg_achv_rwd() RETURNS TRIGGER AS $_PGFN_$
DECLARE
  v_reward record;
...
BEGIN
  ...
  FROM myapp_events_public.app_achievement_rewards AS ar
  WHERE
    ar.level_name = NEW.level_name LOOP
    IF v_reward.reward_type = 'limit_credit' THEN
      ...
      IF v_default_limit_id IS NOT NULL THEN
        ...
        VALUES
          (v_default_limit_id, NEW.actor_id, v_reward.amount, v_reward.credit_type, 'achievement:' || NEW.level_name);
      END IF;
    END IF;
  END LOOP;
  RETURN NEW;
